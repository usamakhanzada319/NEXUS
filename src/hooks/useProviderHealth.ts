...
export const useProviderHealth = (): useProviderHealthReturn => {
  ...
  const runHealthCheck = useCallback(
    async (providerId: string) => {
      ...
      try {
        ...
        if (result.success) {
          addNotification(
            `Health check passed for ${result.providerId}`,
            "success",
          );
        } else {
          addNotification(
            `Health check failed for ${result.providerId}`,
            "error",
          );
        }
        return result;
        ...
        if (config) {
          ...
          if (updated) {
            setFallbackConfigs((prev) =>
              prev.map((c) => (c.providerId === providerId ? updated : c)),
            );
            addNotification(
  ...
  const updateHealth = useCallback(
    async (providerId: string, updates: Partial<ProviderHealth>) => {
      try {
        const updated = await apiClient.updateProviderHealth(
          providerId,
          updates,
        );
        ...
        if (updated) {
          setHealthData((prev) =>
            prev.map((h) => (h.providerId === providerId ? updated : h)),
          );
        }
      } catch (error) {
